...
def filter_large_examples(df):
    ...
    for index, row in df.iterrows():
        ctx_tokens = [w.text for w in nlp(row.context, disable=['parser','ner','tagger','lemmatizer'])]
        if len(ctx_tokens)>400:
            ctx_lens.append(row.name)

        query_tokens = [w.text for w in nlp(row.question, disable=['parser','tagger','ner','lemmatizer'])]
        if len(query_tokens)>50:
            query_lens.append(row.name)

        ans_tokens = [w.text for w in nlp(row.answer, disable=['parser','tagger','ner','lemmatizer'])]
...
def build_word_vocab(vocab_text):
    ...
    for sent in vocab_text:
        for word in nlp(sent, disable=['parser','tagger','ner','lemmatizer']):
...
def context_to_ids(text, word2idx):
    ...
    context_tokens = [w.text for w in nlp(text, disable=['parser','tagger','ner','lemmatizer'])]
...
def question_to_ids(text, word2idx):
    ...
    question_tokens = [w.text for w in nlp(text, disable=['parser','tagger','ner','lemmatizer'])]
...
def index_answer(row, idx2word):
    ...
    context_span = [(word.idx, word.idx + len(word.text)) for word in nlp(row.context, disable=['parser','tagger','ner','lemmatizer'])]
    starts, ends = zip(*context_span)
